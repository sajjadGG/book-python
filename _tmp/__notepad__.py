...
def average(args: List[float]) -> Optional[float]:
...
for sepal_length, sepal_width, petal_length, petal_width, species in data:
    values['Sepal length'].append(sepal_length)
    values['Sepal width'].append(sepal_width)
    values['Petal length'].append(petal_length)
    values['Petal width'].append(petal_width)
    values['Species'].append(species)

for name in header:
    averages[name] = average(values[name])
