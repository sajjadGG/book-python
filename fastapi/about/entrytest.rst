About Entry Test
================


Assignments
-----------
.. literalinclude:: assignments/about_entrytest_a.py
    :caption: Entry Test Assignment A
    :end-before: # Solution

.. literalinclude:: assignments/about_entrytest_b.py
    :caption: Entry Test Assignment B
    :end-before: # Solution

.. literalinclude:: assignments/about_entrytest_c.py
    :caption: Entry Test Assignment C
    :end-before: # Solution

.. literalinclude:: assignments/about_entrytest_d.py
    :caption: Entry Test Assignment D
    :end-before: # Solution

.. literalinclude:: assignments/about_entrytest_e.py
    :caption: Entry Test Assignment E
    :end-before: # Solution
